...
CREATE TABLE IF NOT EXISTS activation_codes (
    ...
    PRIMARY KEY (username, membership)
)
''')
...
class BattleApp:
    ...
    def assign_code_to_user(self):
        username = self.user_entry.get()
        membership = self.membership_var.get()
        code = self.code_entry.get()
        
        cursor.execute('INSERT OR REPLACE INTO activation_codes (username, membership, activation_code) VALUES (?, ?, ?)', (username, membership, code))
        conn.commit()
        
        messagebox.showinfo("Success", f"Activation code assigned for {username} ({membership})")
        self.log_event(f"Activation code assigned for {username} ({membership}): {code}")
    ...
    def show_tasks(self):
        if self.current_player:
            ...
            tk.Label(tasks_window, text="Tasks and Assignments", font=("Helvetica", 16)).pack(padx=10, pady=10)
            tk.Label(tasks_window, text="1. Complete your daily challenges.", font=("Helvetica", 12)).pack(padx=10, pady=5)
            tk.Label(tasks_window, text="2. Participate in battles to increase your win rate.", font=("Helvetica", 12)).pack(padx=10, pady=5)

    def show_advertising(self):
        if self.current_player:
            ad_window = tk.Toplevel(self.root)
            ad_window.title("Advertising")
            tk.Label(ad_window, text="Advertising", font=("Helvetica", 16)).pack(padx=10, pady=10)
            tk.Label(ad_window, text="Check out our latest offers and updates!", font=("Helvetica", 12)).pack(padx=10, pady=5)
    ...
    def show_admin_panel(self):
        if self.current_player and self.current_player.is_admin:
            ...
            tk.Label(admin_panel, text="Admin Panel", font=("Helvetica", 16)).pack(padx=10, pady=10)

            # Admin Panel UI
            tk.Button(admin_panel, text="View Player Stats", command=self.view_player_stats).pack(padx=10, pady=10)
            tk.Button(admin_panel, text="Assign Activation Code", command=self.show_assign_code_panel).pack(padx=10, pady=10)
            tk.Button(admin_panel, text="Log Out", command=self.logout).pack(padx=10, pady=10)
            tk.Button(admin_panel, text="Promote User", command=self.promote_user).pack(padx=10, pady=10)
            tk.Button(admin_panel, text="Add New Script", command=self.add_new_script).pack(padx=10, pady=10)

    def view_player_stats(self):
        if self.current_player and self.current_player.is_admin:
            stats_window = tk.Toplevel(self.root)
            stats_window.title("Player Stats")
            tk.Label(stats_window, text="Player Statistics", font=("Helvetica", 16)).pack(padx=10, pady=10)

            self.stats_listbox = tk.Listbox(stats_window, width=50, height=15)
            self.stats_listbox.pack(padx=10, pady=10)

            cursor.execute('SELECT username, membership, games_played, win_rate FROM players')
            players = cursor.fetchall()
            for player in players:
                self.stats_listbox.insert(tk.END, f"Username: {player[0]}, Membership: {player[1]}, Games Played: {player[2]}, Win Rate: {player[3]:.2%}")

    def show_assign_code_panel(self):
        if self.current_player and self.current_player.is_admin:
            assign_code_window = tk.Toplevel(self.root)
            assign_code_window.title("Assign Activation Code")

            tk.Label(assign_code_window, text="Assign Activation Code", font=("Helvetica", 16)).pack(padx=10, pady=10)

            tk.Label(assign_code_window, text="Username:").pack(padx=10, pady=5)
            self.user_entry = tk.Entry(assign_code_window)
            self.user_entry.pack(padx=10, pady=5)

            tk.Label(assign_code_window, text="Membership:").pack(padx=10, pady=5)
            self.membership_var = tk.StringVar(value="Junior")
            membership_menu = tk.OptionMenu(assign_code_window, self.membership_var, *scripts.keys())
            membership_menu.pack(padx=10, pady=5)

            tk.Label(assign_code_window, text="Activation Code:").pack(padx=10, pady=5)
            self.code_entry = tk.Entry(assign_code_window)
            self.code_entry.pack(padx=10, pady=5)

            tk.Button(assign_code_window, text="Assign Code", command=self.assign_code_to_user).pack(padx=10, pady=10)
    ...
    def assign_code_to_user(self):
        ...
        cursor.execute('INSERT OR REPLACE INTO activation_codes (username, membership, activation_code) VALUES (?, ?, ?)',
                       (username, membership, code))
        conn.commit()
        messagebox.showinfo("Success", f"Activation code assigned for {username} ({membership})")
        self.log_event(f"Activation code assigned for {username} ({membership}): {code}")
    ...
    def promote_user(self):
        if self.current_player and self.current_player.is_admin:
            ...
            promote_window.title("Promote User")
            
            tk.Label(promote_window, text="Username:").pack(padx=10, pady=5)
            user_entry = tk.Entry(promote_window)
            user_entry.pack(padx=10, pady=5)
            
            tk.Label(promote_window, text="New Membership Level:").pack(padx=10, pady=5)
            membership_var = tk.StringVar(value="Junior")
            membership_menu = tk.OptionMenu(promote_window, membership_var, *scripts.keys())
            membership_menu.pack(padx=10, pady=5)
            
            tk.Button(promote_window, text="Promote User", command=lambda: self.update_membership(user_entry.get(), membership_var.get())).pack(padx=10, pady=10)
    def add_new_script(self):
        if self.current_player and self.current_player.is_admin:
            add_script_window = tk.Toplevel(self.root)
            add_script_window.title("Add New Script")
            
            tk.Label(add_script_window, text="Script Name:").pack(padx=10, pady=5)
            script_name_entry = tk.Entry(add_script_window)
            script_name_entry.pack(padx=10, pady=5)
            
            tk.Label(add_script_window, text="Script Description:").pack(padx=10, pady=5)
            script_desc_entry = tk.Entry(add_script_window)
            script_desc_entry.pack(padx=10, pady=5)
            
            tk.Button(add_script_window, text="Add Script", command=lambda: self.insert_new_script(script_name_entry.get(), script_desc_entry.get())).pack(padx=10, pady=10)

def insert_new_script(self, name, description):
    scripts[name] = description
    messagebox.showinfo("Success", f"New script '{name}' added.")

def update_membership(self, username, new_membership):
    ...
    def logout(self):
        self.current_player = None
        self.login_frame.grid(row=0, column=0, padx=10, pady=10, fill="x")
        self.battle_frame.pack_forget()
        self.log_event("Logged out")
